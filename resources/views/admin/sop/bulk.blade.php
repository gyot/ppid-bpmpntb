...
@section('title', 'Upload Massal SOP')
...
@section('content')
<div class="max-w-4xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Upload Massal SOP</h1>

    <form action="{{ route('admin.sop.bulk-store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf

        <div class="card bg-white rounded-lg shadow p-6 space-y-4">
            <h2 class="text-lg font-semibold text-gray-900 border-b pb-2">Pengaturan SOP</h2>
            <p class="text-sm text-gray-500">Judul dan deskripsi otomatis dari nama file. Isi deskripsi manual jika ingin menimpa.</p>

            <div>
                <label class="input-label block text-sm font-medium text-gray-700 mb-1">Deskripsi (opsional)</label>
                <textarea name="deskripsi" rows="3" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('deskripsi') border-red-500 @enderror" placeholder="Kosongkan untuk menggunakan judul sebagai deskripsi">{{ old('deskripsi') }}</textarea>
