...
@section('title', 'Edit Dokumen Keuangan')
...
@section('content')
<div class="max-w-4xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Edit Dokumen Keuangan</h1>

    <form action="{{ route('admin.keuangan.update', $item) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="card bg-white rounded-lg shadow p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="category" required class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
                        <option value="laporan_keuangan" {{ old('category', $item->category) == 'laporan_keuangan' ? 'selected' : '' }}>Laporan Keuangan</option>
                        <option value="rka" {{ old('category', $item->category) == 'rka' ? 'selected' : '' }}>RKA</option>
                        <option value="dipa" {{ old('category', $item->category) == 'dipa' ? 'selected' : '' }}>DIPA</option>
                        <option value="realisasi" {{ old('category', $item->category) == 'realisasi' ? 'selected' : '' }}>Realisasi</option>
                        <option value="calk" {{ old('category', $item->category) == 'calk' ? 'selected' : '' }}>CALK</option>
                        <option value="neraca" {{ old('category', $item->category) == 'neraca' ? 'selected' : '' }}>Neraca</option>
                        <option value="arus_kas" {{ old('category', $item->category) == 'arus_kas' ? 'selected' : '' }}>Arus Kas</option>
                    </select>
                </div>
                <div>
