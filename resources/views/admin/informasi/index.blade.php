...
@section('breadcrumb')
    <span class="text-gray-900">Informasi Publik</span>
@endsection

@section('content')
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h1 class="text-2xl font-bold text-gray-900">Informasi Publik</h1>
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.informasi.create') }}" class="btn-primary inline-flex items-center px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 text-sm">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                Tambah Informasi
            </a>
            <a href="{{ route('admin.informasi.bulk-create') }}" class="inline-flex items-center px-4 py-2 border border-primary text-primary rounded-lg hover:bg-primary/5 text-sm">
    ...
    <div class="card bg-white rounded-lg shadow p-4">
        <form method="GET" class="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari informasi..." class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
            ...
            <div class="sm:col-span-4 flex gap-2">
                ...
                <a href="{{ route('admin.informasi.index') }}" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm">Reset</a>
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="table-custom w-full text-sm">
                ...
                <tbody>
                        ...
                        <tr class="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.informasi.edit', $item) }}" class="text-blue-600 hover:text-blue-800" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form method="POST" action="{{ route('admin.informasi.toggle-status', $item) }}" class="inline">
                                        @csrf @method('PATCH')
                                    ...
                                    <button onclick="confirmDelete('{{ route('admin.informasi.destroy', $item) }}')" class="text-red-600 hover:text-red-800" title="Hapus">
                    ...
                    @empty
                        <tr><td colspan="6" class="py-8 text-center text-gray-400">Tidak ada data informasi</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($informasi->hasPages())
            <div class="px-4 py-3 border-t border-gray-100">{{ $informasi->withQueryString()->links() }}</div>
        @endif
    </div>
</div>
