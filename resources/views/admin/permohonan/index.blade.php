...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h1 class="text-2xl font-bold text-gray-900">Permohonan Informasi</h1>
    </div>
    ...
    <div class="card bg-white rounded-lg shadow p-4">
        <div class="flex flex-wrap gap-2">
            ...
            @endphp
            @foreach($tabs as $tab)
                <a href="{{ route('admin.permohonan.index', ['status' => $tab['value']]) }}"
                        ...
                        <tr class="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4">
                                <div class="flex items-center justify-end">
                                    <a href="{{ route('admin.permohonan.show', $item) }}" class="text-primary hover:text-primary/80 text-sm font-medium">Detail</a>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="py-8 text-center text-gray-400">Tidak ada permohonan</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($permohonan->hasPages())
            <div class="px-4 py-3 border-t border-gray-100">{{ $permohonan->withQueryString()->links() }}</div>
        @endif
    </div>
</div>
@endsection
