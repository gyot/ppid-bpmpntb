...
@section('content')
<div class="max-w-4xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Tambah Data Pengadaan</h1>

    <form action="{{ route('admin.pengadaan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="card bg-white rounded-lg shadow p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nama_paket" value="{{ old('nama_paket') }}" required class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('nama_paket') border-red-500 @enderror">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="tahap" required class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
                        <option value="rencana" {{ old('tahap') == 'rencana' ? 'selected' : '' }}>Rencana</option>
                        <option value="pemilihan" {{ old('tahap') == 'pemilihan' ? 'selected' : '' }}>Pemilihan</option>
                        <option value="pelaksanaan" {{ old('tahap') == 'pelaksanaan' ? 'selected' : '' }}>Pelaksanaan</option>
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="nilai_pagu" value="{{ old('nilai_pagu') }}" step="0.01" min="0" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="penyedia" value="{{ old('penyedia') }}" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
                </div>
                <div>
                    <label class="input-label block text-sm font-medium text-gray-700 mb-1">No. Kontrak</label>
                    <input type="text" name="no_kontrak" value="{{ old('no_kontrak') }}" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
                </div>
            </div>
