...
@section('content')
<div class="max-w-4xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Edit Data LHKPN</h1>

    <form action="{{ route('admin.lhkpn.update', $item) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="card bg-white rounded-lg shadow p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nama_pejabat" value="{{ old('nama_pejabat', $item->nama_pejabat) }}" required class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('nama_pejabat') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="text" name="jabatan" value="{{ old('jabatan', $item->jabatan) }}" required class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('jabatan') border-red-500 @enderror">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="nip" value="{{ old('nip', $item->nip) }}" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('nip') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <select name="periode" required class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('periode') border-red-500 @enderror">
                        @for($y = date('Y'); $y >= 2020; $y--)
                            <option value="{{ $y }}" {{ old('periode', $item->periode) == $y ? 'selected' : '' }}>{{ $y }}</option>
                        @endfor
                    </select>
            ...
            <div>
                <label class="input-label block text-sm font-medium text-gray-700 mb-1">File LHKPN</label>
                @if($item->file_path)
                    <p class="text-sm text-gray-500 mb-1">File saat ini: <a href="{{ Storage::url($item->file_path) }}" target="_blank" class="text-primary hover:underline">{{ $item->file_name }}</a></p>
                @endif
