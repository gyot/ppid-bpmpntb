...
@section('breadcrumb')
    <a href="{{ route('admin.keberatan.index') }}" class="hover:text-primary">Keberatan</a>
    ...
    <span class="text-gray-900">{{ $keberatan->registration_number }}</span>
@endsection

@section('content')
<div class="space-y-6" x-data="{ showProcess: false, showComplete: false, showReject: false }">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h1 class="text-2xl font-bold text-gray-900">Detail Keberatan</h1>
        <div class="flex flex-wrap gap-2">
            ...
            @endif
            @if(in_array($keberatan->status, ['baru', 'diproses']))
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 card bg-white rounded-lg shadow p-6">
            <h2 class="text-lg font-semibold text-gray-900 border-b pb-3 mb-4">Informasi Keberatan</h2>
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                <div>
                    <dt class="text-xs text-gray-500 uppercase tracking-wider">No. Registrasi</dt>
                    <dd class="text-sm font-medium text-gray-900 mt-0.5">{{ $keberatan->registration_number }}</dd>
                </div>
                <div>
                    ...
                    <dd class="mt-0.5">
                        ...
                        @endphp
                        <span class="badge-{{ $statusColors[$keberatan->status] ?? 'info' }} text-xs px-2 py-1 rounded-full">{{ ucfirst($keberatan->status) }}</span>
                    </dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-500 uppercase tracking-wider">Nama Pemohon</dt>
                    <dd class="text-sm text-gray-900 mt-0.5">{{ $keberatan->name }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-500 uppercase tracking-wider">Email</dt>
                    <dd class="text-sm text-gray-900 mt-0.5">{{ $keberatan->email }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-500 uppercase tracking-wider">Telepon</dt>
                    <dd class="text-sm text-gray-900 mt-0.5">{{ $keberatan->phone ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-500 uppercase tracking-wider">Permohonan Terkait</dt>
                    <dd class="text-sm text-gray-900 mt-0.5">{{ $keberatan->permohonan->registration_number ?? '-' }}</dd>
                </div>
                <div class="sm:col-span-2">
                    <dt class="text-xs text-gray-500 uppercase tracking-wider">Alasan Keberatan</dt>
                    <dd class="text-sm text-gray-900 mt-0.5">{{ $keberatan->reason }}</dd>
                </div>
                <div class="sm:col-span-2">
                    <dt class="text-xs text-gray-500 uppercase tracking-wider">Rincian Keberatan</dt>
                    <dd class="text-sm text-gray-900 mt-0.5">{{ $keberatan->detail ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-500 uppercase tracking-wider">Tanggal Pengajuan</dt>
                    <dd class="text-sm text-gray-900 mt-0.5">{{ $keberatan->created_at->format('d M Y H:i') }}</dd>
                </div>
            </dl>
        </div>
        ...
        <div class="card bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-4">
                @forelse($keberatan->statusHistory ?? [] as $history)
    ...
    <div x-show="showProcess" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" x-transition>
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4" @click.away="showProcess = false">
            <form method="POST" action="{{ route('admin.keberatan.review', $keberatan) }}">
                @csrf @method('PATCH')
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Proses Keberatan</h3>
                    <div>
    ...
    <div x-show="showComplete" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" x-transition>
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4" @click.away="showComplete = false">
            <form method="POST" action="{{ route('admin.keberatan.respond', $keberatan) }}">
                @csrf @method('PATCH')
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Selesaikan Keberatan</h3>
                    <div>
                        <label class="input-label block text-sm font-medium text-gray-700 mb-1">Catatan</label>
                        <textarea name="notes" rows="3" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary"></textarea>
                    </div>
    ...
    <div x-show="showReject" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" x-transition>
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4" @click.away="showReject = false">
            <form method="POST" action="{{ route('admin.keberatan.resolve', $keberatan) }}">
                @csrf @method('PATCH')
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-red-600 mb-4">Tolak Keberatan</h3>
                    <div>
