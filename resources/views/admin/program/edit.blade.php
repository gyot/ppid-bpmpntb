...
@section('content')
<div class="max-w-4xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Edit Program / Kegiatan</h1>

    <form action="{{ route('admin.program.update', $item) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="card bg-white rounded-lg shadow p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nama_program" value="{{ old('nama_program', $item->nama_program) }}" required class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('nama_program') border-red-500 @enderror">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="jenis" required class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
                        <option value="program" {{ old('jenis', $item->jenis) == 'program' ? 'selected' : '' }}>Program</option>
                        <option value="kegiatan" {{ old('jenis', $item->jenis) == 'kegiatan' ? 'selected' : '' }}>Kegiatan</option>
                        <option value="strategis" {{ old('jenis', $item->jenis) == 'strategis' ? 'selected' : '' }}>Strategis</option>
                    </select>
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="penanggung_jawab" value="{{ old('penanggung_jawab', $item->penanggung_jawab) }}" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
                </div>
                <div>
                    <label class="input-label block text-sm font-medium text-gray-700 mb-1">Target</label>
                    <input type="text" name="target" value="{{ old('target', $item->target) }}" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="jadwal" value="{{ old('jadwal', $item->jadwal) }}" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
                </div>
                <div>
                    <label class="input-label block text-sm font-medium text-gray-700 mb-1">Sumber Anggaran</label>
                    <input type="text" name="sumber_anggaran" value="{{ old('sumber_anggaran', $item->sumber_anggaran) }}" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
                </div>
            </div>

            <div>
                <label class="input-label block text-sm font-medium text-gray-700 mb-1">Besaran Anggaran (Rp)</label>
                <input type="number" name="besaran_anggaran" value="{{ old('besaran_anggaran', $item->besaran_anggaran) }}" step="0.01" min="0" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
            </div>

            <div>
                <label class="input-label block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
                <textarea name="deskripsi" rows="4" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">{{ old('deskripsi', $item->deskripsi) }}</textarea>
            </div>
