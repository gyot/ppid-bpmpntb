...
@section('title', 'Kelola DIP')

@section('breadcrumb')
    <span class="text-gray-900">Daftar Informasi Publik</span>
@endsection
...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <a href="{{ route('admin.dip.create') }}" class="btn-primary inline-flex items-center px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 text-sm">
    ...
    <div class="card bg-white rounded-lg shadow p-4">
        <form method="GET" class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari DIP..." class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary">
            ...
            <div class="sm:col-span-3 flex gap-2">
                ...
                <a href="{{ route('admin.dip.index') }}" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm">Reset</a>
            </div>
        </form>
    </div>
            ...
            <table class="table-custom w-full text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="text-left py-3 px-4 font-medium text-gray-500">Sumber</th>
                ...
                <tbody>
                    @forelse($dip as $item)
                        <tr class="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4 text-sm text-gray-600">{{ Str::limit($item->jenis_informasi, 30) }}</td>
                            <td class="py-3 px-4 text-sm text-gray-600">{{ $item->sumber_informasi ?? '-' }}</td>
                            <td class="py-3 px-4">
                                <span class="badge-{{ $item->status === 'published' ? 'success' : 'warning' }} text-xs px-2 py-1 rounded-full">{{ ucfirst($item->status) }}</span>
                            </td>
                            <td class="py-3 px-4">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.dip.edit', $item) }}" class="text-blue-600 hover:text-blue-800" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form method="POST" action="{{ route('admin.dip.toggle-status', $item) }}" class="inline">
                                        @csrf
                                        <button type="submit" class="{{ $item->status === 'published' ? 'text-yellow-600 hover:text-yellow-800' : 'text-green-600 hover:text-green-800' }}" title="{{ $item->status === 'published' ? 'Unpublish' : 'Publish' }}">
                                            <svg class="w-4 h-4" fill="none" stroke="Status" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                        </button>
                                    </form>
                                    <button onclick="confirmDelete('{{ route('admin.dip.destroy', $item) }}')" class="text-red-600 hover:text-red-800" title="Hapus">
                    ...
                    @empty
                        <tr><td colspan="6" class="py-8 text-center text-gray-400">Tidak ada data DIP</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($dip->hasPages())
            <div class="px-4 py-3 border-t border-gray-100">{{ $dip->withQueryString()->links() }}</div>
        @endif
    </div>
</div>
