...
@section('content')
<div class="max-w-4xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Edit Daftar Informasi Publik</h1>

    <form action="{{ route('admin.dip.update', $item) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="card bg-white rounded-lg shadow p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="category" required class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('category') border-red-500 @enderror">
                        ...
                        <option value="berkala" {{ old('category', $item->category) == 'berkala' ? 'selected' : '' }}>Berkala</option>
                        <option value="setiap_saat" {{ old('category', $item->category) == 'setiap_saat' ? 'selected' : '' }}>Setiap Saat</option>
                        <option value="serta_merta" {{ old('category', $item->category) == 'serta_merta' ? 'selected' : '' }}>Serta Merta</option>
                        <option value="dikecualikan" {{ old('category', $item->category) == 'dikecualikan' ? 'selected' : '' }}>Dikecualikan</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="jenis_informasi" value="{{ old('jenis_informasi', $item->jenis_informasi) }}" required class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('jenis_informasi') border-red-500 @enderror">
            ...
            <div>
                ...
                <textarea name="uraian_informasi" rows="4" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('uraian_informasi') border-red-500 @enderror">{{ old('uraian_informasi', $item->uraian_informasi) }}</textarea>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="sumber_informasi" value="{{ old('sumber_informasi', $item->sumber_informasi) }}" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('sumber_informasi') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="text" name="media_informasi" value="{{ old('media_informasi', $item->media_informasi) }}" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('media_informasi') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="text" name="jkd" value="{{ old('jkd', $item->jkd) }}" class="input-field w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary @error('jkd') border-red-500 @enderror">
            ...
            <div>
                ...
                @if($item->file_path)
                    <p class="text-sm text-gray-500 mb-1">File saat ini: <a href="{{ Storage::url($item->file_path) }}" target="_blank" class="text-primary hover:underline">{{ $item->file_name ?? basename($item->file_path) }}</a></p>
                @endif
