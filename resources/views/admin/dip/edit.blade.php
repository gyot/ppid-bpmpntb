    ...
    <form action="{{ route('admin.dip.update', $item) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="card bg-white rounded-lg shadow p-6 space-y-4">
            ...
            <div>
                ...
                @if($item->file_path)
                    <p class="text-sm text-gray-500 mb-1">File saat ini: <a href="{{ '/' . $item->file_path }}" target="_blank" class="text-primary hover:underline">{{ $item->file_name ?? basename($item->file_path) }}</a></p>
                @endif
