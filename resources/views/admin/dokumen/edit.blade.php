...
@section('content')
<div class="max-w-4xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Edit Dokumen</h1>

    <form action="{{ route('admin.dokumen.update', $item) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="card bg-white rounded-lg shadow p-6 space-y-4">
            ...
            <div>
                ...
                @if($item->file_path)
                    <p class="text-sm text-gray-500 mb-1">File saat ini: <a href="{{ Storage::url($item->file_path) }}" target="_blank" class="text-primary hover:underline">{{ basename($item->file_path) }}</a></p>
                @endif
